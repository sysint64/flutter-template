...
void main() {
  test('parse primitive', () async {
    expect(FieldType(isPrimitive: true, name: 'int', displayName: 'int'),
        parseType('int'));
    expect(FieldType(isPrimitive: true, name: 'String', displayName: 'String'),
        parseType('String'));
    expect(FieldType(isPrimitive: true, name: 'bool', displayName: 'bool'),
        parseType('bool'));
    expect(FieldType(isPrimitive: true, name: 'num', displayName: 'num'),
        parseType('num'));
    expect(FieldType(isPrimitive: true, name: 'num', displayName: 'num'),
        parseType(' num'));
  });
  ...
  test('parse non primitive', () async {
    expect(FieldType(isPrimitive: false, name: 'Box', displayName: 'Box'),
        parseType('Box'));
  });
  ...
  test('tokenize compound with spaces 2', () async {
    expect(
      tokenize(StringStream(
          '  Optional<  Map < App_Item21, Optional  < _Item015>   >  > ')),
      [
        Token(tokenType: TokenType.id, data: 'Optional'),
        Token(tokenType: TokenType.symbol, data: '<'),
        Token(tokenType: TokenType.id, data: 'Map'),
