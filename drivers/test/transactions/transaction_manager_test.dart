...
class MockTransactionStateStorage extends Mock
    implements TransactionStateStorage {}

class MockTransactionFactory extends Mock implements TransactionFactory {}
...
void main() {
  test('test cancel', () async {
    ...
    when(transaction.restoreStateFromBackup())
  ...
  test('test recoveyState', () async {
    ...
    when(transactionFactory.createTransactionById('T1'))
        .thenAnswer((_) => Future.value(t1));

    when(transactionFactory.createTransactionById('T2'))
        .thenAnswer((_) => Future.value(t2));
